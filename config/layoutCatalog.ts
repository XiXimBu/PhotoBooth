...
/** Đo ô trên canvas mẫu (pixel) → tỉ lệ 0→1 (cùng công thức với seed cũ) */
function normSlot(
  canvasW: number,
  canvasH: number,
  x: number,
...
/**
 * Nguồn dữ liệu duy nhất cho: seed DB, preview trang camera, API khung/ghép ảnh.
 * Sửa layout tại đây rồi chạy lại seed (hoặc chỉ deploy — runtime lấy geometry từ catalog).
 */
export const LAYOUT_CATALOG: LayoutCatalogEntry[] = [
  ...
  },
  {
    ...
    photoCount: 4,
    // Khung PNG thường 707×1644; canvas mẫu 707×2000 — preview/API dùng contain khớp ô.
    canvasW: 707,
    canvasH: 2000,
    slots: [
...
const catalogByName = new Map<string, LayoutCatalogEntry>();
for (const entry of LAYOUT_CATALOG) {
  catalogByName.set(entry.name.trim(), entry);
}

export function getCatalogLayoutByName(
  name: string
): LayoutCatalogEntry | undefined {
  return catalogByName.get(String(name).trim());
}

/**
 * Ghi đè canvas/slots từ catalog theo `name` (document layout từ Mongo có thể lệch version seed).
 */
export function applyCatalogToLayoutDoc(
  layout: unknown
): unknown {
  ...
  return {
    ...
    slots: cat.slots,
    aspectRatio: aspectRatioLabel(cat.canvasW, cat.canvasH),
  };
}
